...
def is_wsl() -> bool:
    ...
    return re.search("microsoft", platform.uname().release, re.IGNORECASE) is not None
...
class SSHConfigEntry:
    ...
    def __init__(self, json: list[dict]):
        ...
        if self.url is None:
            raise ValueError(
                f"Key does not specify a URL to connect to! Add a URL field to the 1Password entry for {self.keyname}"
            )
...
def make_config(entries: list[SSHConfigEntry], path: str) -> None:
    ...
    if os.path.exists(path):
        ...
        new_loc = shutil.make_archive(
            base_name=f'{path}-{time.strftime("%Y%m%d-%H%M%S")}.old',
            format="gztar",
            root_dir=path,
        )
        print(
            f'Moving old version of .ssh directory to a backup location at "{new_loc}"'
        )
        shutil.rmtree(path)
    ...
    with open(os.path.join(path, "config"), "w") as fout:
        fout.write(
            "# This file was automatically generated by a Chezmoi script. Changes will be overwritten if `chezmoi apply` or `chezmoi update` is ran.\n"
        )
        fout.write(
            "# You can find the script somewhere in the directory opened with `chezmoi cd`.\n\n"
        )
...
def main():
    if not question("Generate an SSH directory from 1Password? (Y/n) "):
        sys.exit(0)
    ...
    if OP_EXE is None:
        print(
            "1Password CLI not installed! You can rerun this script with `chezmoi update`. Quitting.",
            file=sys.stderr,
        )
        if is_wsl():
            print(
                "Hint: If you're trying to use WSL, you need the Windows CLI installed, not Linux!",
                file=sys.stderr,
            )
        sys.exit(0)
    ...
    if os.path.exists(path):
        if not question(
            "This will replace your existing directory, but a backup will be made. Continue? (Y/n) "
        ):
            sys.exit(0)
        known_hosts_path = os.path.join(path, "known_hosts")
        if os.path.exists(known_hosts_path):
            if question(
                'Preserve known_hosts file? Recommended to answer "n" if you don\'t know what this is. (Y/n) '
            ):
                with open(known_hosts_path, "r") as fin:
                    known_hosts = fin.read()

    # Get all SSH Key entries by ID
    get_entries_res = call_shell(
        f"{OP_EXE} item list --categories SSHKEY --format json"
    )
    key_ids = []
    for key in json.loads(get_entries_res):
        ...
        if re.search("signing", key["title"], re.IGNORECASE):
            if not question(
                f'Key "{key['title']}" appears to be for signing. Load anyways? (y/N) ',
                default="n",
            ):
                continue
    ...
    for key in key_ids:
        res = call_shell(
            f'{OP_EXE} item get --format json --fields label="public key",label="chezmoi params" {key}'
        )
